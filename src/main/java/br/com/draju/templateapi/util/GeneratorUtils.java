		...
		try {
			...
			for (Field f : aClassFields) {
				if (Modifier.isPrivate(f.getModifiers())) {
					...
					log.debug("Making field {} accessible, and its value is: {}", f.getName(), f.get(dtoObject));
				}
				...
				if (f.getType().toString().contains("java.lang.String")) {
					...
					if(f.get(dtoObject) != null) {
						log.info(" putting enum value in map {} = {} ", fName, f.get(dtoObject));
						map.put(fName, (String) f.get(dtoObject).toString());
					}
				} else if (f.getType().toString()
						.contains("br.com.draju.templateapi.entity.actiondata.ActionArgument")) {
